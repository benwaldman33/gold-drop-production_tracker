...
- **Material journey viewer** - the app now includes an in-app HTML genealogy viewer at `/journeys/material-genealogy` with `By Lot` and `By Run` modes, so managers can trace a derivative lot back to source biomass or open a run and follow its derivative outputs without dropping into raw JSON endpoints.
