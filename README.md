...
- **Standalone Extraction Lab App** — `standalone-extraction-lab-app/` now mirrors the extractor workflow with a touch-first reactor board, a `Scan / Enter Lot` surface, chargeable lot browser, `100 lbs` default charge preset, remembered reactor defaults, and a standalone run-execution screen with booth-SOP progression checkpoints from vacuum confirmation through shutdown and run completion. `Settings -> Operational Parameters` can now prepopulate that run screen's common defaults for blend, fill / flush counts, total fill / flush weights, baskets, and CRC blend.
- **Extraction booth SOP workflow** — booth execution is now modeled separately from the downstream business summary on `Run`. Operators can advance through vacuum confirmation, solvent charge, soak, mixer, filter clear, pressurization, recovery, flush setup, temperature verification, flush solvent charge, flow-resumed confirmation, final purge, final clarity, shutdown checklist, and run completion.
...
- **Deviation reasons + supervisor override controls** — off-target booth timing and exception paths now require an operator reason, and supervisors can explicitly `Approve Deviation` or `Require Rework` with a recorded reason on the dashboard notification card.
