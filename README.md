...
**Access-control notes** live in `ACCESS_CONTROL.md`.
**FAQ** lives in `FAQ.md`.
...
There is no separate seeded **extractor** username. Floor-operator extraction testing uses **`ops`** (or any **`user`**-role account your administrator creates). The standalone extraction app shows the **operator run dashboard** for `extractor`, `assistant_extractor`, and `user` roles; `manager`, `supervisor`, `admin`, and `vp_operations` get the broader supervisor run layout.
...
- **Settings subpages** - Super Admin settings now open as focused pages for Operational Parameters, Journey Financials, Extraction Controls, Slack & Notifications, Users & Access, Access Control, Field Intake, API Clients, Scales, Remote Sites, and Maintenance instead of one crowded settings screen.
- **Access Control** - `Settings -> Access Control` lets Super Admins manage role permission templates and per-user grants/revokes separately from user creation, including standalone app access and action-level import/export controls.
- **Alerts Home** - a new `/alerts` hub summarizes supervisor notifications and active genealogy issues as an action queue.
- **Supervisor Console** - a new `/supervisor-console` surface under Downstream gives supervisors a launch-ready hub for operational role coverage, queue health, open alerts, blocked/stale downstream work, and handoffs into Floor Ops, Alerts, Journey, and destination queues.
...
- **Manager Journey Dashboard** - Journey Home now surfaces blocked work, critical issues, aging derivative lots, low-margin runs, open inventory value leaders, and 7/30 day projected revenue and margin based on genealogy-backed run history.
...
- **Journey actual revenue and variance** - material lots can now store actual revenue events, the Material Journey Viewer can record/correct/void those actuals, and Journey Home / Genealogy Report roll actual revenue, actual margin, and projected-vs-actual variance into manager review surfaces.
- **Finance & Accounting** - `Journey -> Finance & Accounting` turns recorded revenue events and genealogy cost basis into period revenue, estimated COGS, gross margin, product/channel summaries, projected inventory value, financial flag counts, and CSV export.
- **Journey financial completeness flags** - Journey Home, Genealogy Report, and Material Journey Viewer now call out missing cost basis, missing revenue assumptions, missing actual revenue on released lots, and open genealogy issues that may affect financial reporting.
- **Journey financial exports and product cards** - Material Genealogy Report can export a financial CSV, and Journey surfaces product-type cards for quantity, cost basis, projected revenue, actual revenue, variance, margin, and flag counts.
...
- **Launch role coverage** - `LAUNCH_ROLE_COVERAGE.md` defines the minimum launch-ready coverage for buyer, receiver/inventory, extractor, and supervisor/downstream roles.
- **Launch Readiness register** - Super Admins can use `Settings -> Launch Readiness` to track acceptance testing, launch blockers, pilot blockers, post-launch work, wishlist items, owners, target dates, and notes before the final data/access security phase.
- **Audit Log manager** - `Settings -> Audit Log` gives audit-capable admins a searchable view of who changed what, when, and why across operating records, imports, exports, corrections, access-control changes, revenue events, and mobile actions.
...
- **Material journey viewer** - the app now includes an in-app HTML genealogy viewer at `/journeys/material-genealogy` with `By Lot` and `By Run` modes, including a live Journey Graphic that maps supplier/source biomass through extraction into derivative product lots before the detailed audit/revenue/reconciliation panels.
- **Revenue capture in the Material Journey Viewer** - editors can record sold quantity, unit price, buyer/channel, reference, and notes against a material lot, then update or void events while preserving the audit trail as a layer separate from planning assumptions.
- **Genealogy correction workflow in context** - the lot view inside the material journey viewer now shows open reconciliation issues, inline correction history, and a direct `Correct This Lot` action that returns the user to the viewer after recording a correction.
- **Run reconciliation in context** - the run view inside the material journey viewer now surfaces run-level genealogy issues, source-allocation exceptions, and links into affected derivative lots so managers can investigate lineage problems from the run outward.
- **Genealogy issue queue** - unresolved genealogy problems now have a dedicated queue with owner assignment, workflow status, working notes, and visible audit history so reconciliation work can be managed instead of handled ad hoc.
- **Genealogy lifecycle automation** - the issue queue now supports explicit resolve/reopen actions with notes, overdue reminder escalation for stale unresolved issues, queue filters by status/severity/owner/age, and correction-form follow-up controls for linked genealogy issues.
- **Genealogy cost and yield reporting** - the genealogy report now surfaces source-lot input quantity vs descendant cost basis, run-level yield/cost review rows, and correction-impact reporting so managers can tie lineage, rework, and rolled-forward cost together.
...
- **Extraction dashboard layout** - the main dashboard now presents period controls, Floor Ops / Runs shortcuts, grouped throughput and on-hand metrics, weekly buyer budget context, supervisor notifications, KPI cards, and quick actions in a more polished operating layout.
...
- **Inventory spreadsheet import** — Upload **.csv**, **.xlsx**, or **.xlsm** via **Inventory -> Import spreadsheet**. The app detects headers, lets authorized import users remap columns interactively, previews matched lots by tracking ID, and then applies only the safe lot-edit fields already supported in the UI: strain, potency, location, floor state, milled state, and notes. It intentionally does **not** create lots or rewrite weights / tracking IDs.
...
- **Post-extraction foundation** — After an extraction run is completed, the same run record now carries the first downstream handoff fields: `post_extraction_pathway` (`100 lb pot pour` vs `200 lb minor run`), `post_extraction_started_at`, `post_extraction_initial_outputs_recorded_at`, and the initial wet THCA / wet HTE outputs. This is the shipped Phase 1 foundation for the later THCA / HTE workflow orchestration.
- **Downstream state tracking** — The shared run record now also carries the first structured Phase 2 downstream fields: pot-pour warm off-gas timing and stir count, THCA oven / milling / destination, and HTE off-gas / clean-dirty / Prescott / queue-routing decisions. This now runs through the standalone extraction app's dedicated `Downstream` queue/tab while keeping the same canonical run data model.
- **Downstream Queues** — The main app now includes a dedicated `Downstream Queues` page that groups completed post-extraction runs into `Needs Queue Decision`, `GoldDrop production queue`, `Liquid Loud hold`, `Terp strip / CDT cage`, `HP base oil hold`, and `Distillate hold`, with move/complete actions for supervisor routing.
- **GoldDrop Production Queue** — The first queue-specific downstream workflow is now live as a dedicated main-app page with queue-state history and supervisor actions for review, production queuing, completion/release, and re-routing.
- **GoldDrop staged workflow** — the GoldDrop queue now behaves like a real production workflow instead of a flat queue. Runs move through review, production queueing, active production, packaging-ready status, and final release, with the UI only exposing the actions that fit the current stage.
...
- **Extraction charge workflow** — A lot can now be charged into production from either the main purchase form (**Charge Lot**) or the scanned-lot workflow. The app records a persisted extraction-charge event with lbs, reactor, charge time, notes, and source mode, confirms the load details before submit, and then supports either **Record Load** (stay in main app) or **Record Load and Open Run**.
...
- **Floor Ops** — A dedicated operator floor page surfaces recent scan activity, recent scale captures, open lot counts, active device counts, floor-state rollups, extraction-readiness counts, and an active reactor board in one place, with a direct **Scan Center** launcher and a now-consistent card layout across summaries, queues, and recent activity sections.
...
- **Active reactor board** — `Floor Ops` now gives each reactor a current state card (`Empty`, `Charged / waiting`, or `Run linked`) with the lot, lbs, charge time, queue depth, operator label, and direct `Open Run` shortcut when a run is already attached.
- **Reactor lifecycle controls** — Extractors can now move a charge through `In Reactor`, `Running`, `Completed`, `Reactor Emptied` (after physical pour-out), or `Cancelled` directly from `Floor Ops`, and every state change is timestamped into audit history on the charge.
- **Configurable lifecycle policy** — `Settings -> Operational Parameters` now lets Super Admin decide which reactor lifecycle states are shown, which are required before later steps, whether `Mark Running` requires a linked run, and whether state history is visible on the board.
- **Floor-context run links** — Opening a run from `Floor Ops` now preserves a return path back to `Floor Ops`, while still offering a direct `Open Runs` option from the run form.
- **Board filters and reactor history** — `Floor Ops` now includes a `Board view` filter (`all`, `active`, `pending`, `running`, `completed today`, `cancelled today`) plus a `Reactor History Today` panel for same-day per-reactor milestones.
- **Standalone Extraction Lab App** — `standalone-extraction-lab-app/` now mirrors the extractor workflow with a touch-first reactor board, a `Scan / Enter Lot` surface, chargeable lot browser, `100 lbs` default charge preset, remembered reactor defaults, and a locked standalone run-execution screen with booth-SOP progression checkpoints from under-vacuum confirmation through shutdown and run completion. Extractors get a focused **operator run dashboard** with one primary action at a time, a compact **run status strip**, and an always-visible **live timers panel** for booth clocks (primary soak, mixer, flush soak, flush mixer, final purge); supervisors/managers get the broader review layout with full timing cards, phase rail, and prep/reset checklist. After run completion, the standalone app routes operators into a dedicated `Downstream` queue/tab for pathway selection, post-extraction handoff, wet-output confirmation, and branch-specific steps. The tablet exposes only the current checkpoint inputs and the next allowed action while the backend rejects future-step progression actions or premature future-step booth fields. `Settings -> Operational Parameters` can prepopulate common run defaults for blend, fill / flush counts, total fill / flush weights, baskets, and CRC blend.
- **Extraction booth SOP workflow** — booth execution is now modeled separately from the downstream business summary on `Run`. Operators advance in strict sequence through under-vacuum confirmation, solvent charge, soak, mixer start, mixer end, primary soak end confirmation, reactor bottom burp confirmation, filter clear, pressurization, recovery, flush setup, temperature verification, flush solvent charge, flush soak with flush-mixer cycle in the last minutes of soak, flow-resumed confirmation, final purge, final clarity, shutdown checklist, and run completion. Future steps stay locked until the current step predicate is satisfied or a manager-approved bypass is applied.
- **Extraction booth evidence** — the standalone extraction app now keeps a collapsible evidence panel available throughout run execution so operators can capture camera/upload proof at any checkpoint. Uploaded solvent-chiller, plate-temperature, and other booth photos are stored as booth evidence records tied to the run/session.
- **Extraction booth timing controls** — booth timers now carry configured target durations for primary soak, mixer, flush soak, flush mixer, and optional final purge. The standalone extraction app now shows live ticking elapsed/remaining time on active timers while still reporting on-target/short status against configured targets.
- **Step 3 wet-output submission reliability** — the standalone run screen no longer nests evidence-upload forms inside the main run form, so `Confirm Initial Outputs` consistently receives the typed wet THCA/HTE values and no longer prompts for missing values that were already entered.
- **Mixer timing safety alerts** — once primary soak starts, the mixer must start within a configurable window (defaults: no sooner than 3 minutes, no later than 6 minutes into soak) and run for a configurable duration (defaults: at least 5 minutes, no more than 7 minutes). The backend issues a critical supervisor notification if the start window is missed or runtime exceeds the maximum, and escalates to an emergency-class Slack alert if the critical alert stays unacknowledged for another 3 minutes.
- **Flush mixer timing during flush soak** — during flush soak, operators start the flush mixer in the last configured minutes (defaults: minute 5 of a 10-minute soak) and run it for 5 minutes. Flush-mixer start/runtime deviations reuse the mixer timing policy. Settings are editable in the standalone app **Settings → Flush Mixer Timing Window** and via `GET/PATCH /api/mobile/v1/extraction/settings`.
- **Extraction booth exception loops** — the booth workflow now supports adjustment/retry states when flow has not resumed yet or final clarity is still not acceptable, instead of forcing the operator into a hard stop.
- **Supervisor booth review surface** — the main run form now exposes a booth-review panel for supervisors with current booth stage, timing status, deviation markers, recent booth history, and linked booth evidence.
- **Supervisor notifications + outbound Slack routing** — booth deviations and run completions now create durable in-app supervisor notifications first, with optional outbound Slack delivery by event class (`completions`, `warnings`, `reminders`) using configurable webhook routing in `Settings -> Slack Integration`.
- **Deviation reasons + supervisor override controls** — off-target booth timing, exception paths, booth-stage bypass requests, and non-admin booth step-back requests now require an operator reason. Supervisors can explicitly `Approve Deviation` or `Require Rework` with a recorded reason on the dashboard notification card; only approved bypass/step-back requests unlock the corresponding one-step action on the tablet.
- **Controlled booth step-back** — operators can move backward only one booth checkpoint at a time. Admin and Super Admin users can apply that one-step rollback directly; other roles must request supervisor approval first. Every request/apply action is logged as booth history, and step-back is blocked once the run is completed.
- **Booth timing policy enforcement** — `Settings -> Operational Parameters` now lets Super Admin choose whether each booth timer is `Informational`, `Warning only`, `Require supervisor override`, or `Hard stop`. Defaults remain permissive so the process can continue by default, while training/intervention cases can be made more strict per step.
- **Supervisor reminder automation** — unresolved warning/critical supervisor alerts can now emit one durable reminder after a configurable delay, with separate thresholds for critical vs warning alerts and optional outbound Slack delivery through the `reminders` webhook route.
...
- **Standalone extraction continuation** — The tablet run screen carries the operator through booth workflow checkpoints; after completion, the reactor view provides a handoff into the standalone app's dedicated `Downstream` queue/tab for post-extraction actions.
- **Standalone downstream capture** — The standalone `Downstream` queue/run mode now carries the first structured downstream fields: pot-pour timing, THCA-path timing and destination, and HTE-path timing / decision / queue fields.
- **Guided downstream workflow** — The tablet downstream flow presents post-extraction work as a numbered, pathway-driven sequence. Operators choose the branch, start post-extraction, enter wet THCA / wet HTE in Step 3, confirm the handoff, then work top-to-bottom through pot-pour or minor-run downstream steps. Start/confirm timestamps are recorded from the step actions, and post-extraction flow now uses step-local actions instead of a page-level save. Pending and completed steps collapse so the active step stays obvious.
- **Runs list completion guardrail** — When `Hide completed` is active on `Runs`, the page now shows a clear reminder and one-click `Show completed runs` action so operators can quickly reveal post-extraction records where wet outputs were captured.
- **Floor Ops wet-output visibility** — The `Active Reactor Board` cards now show linked run wet outputs (HTE / THCA) when a run exists, so extraction supervisors can verify downstream handoff values without leaving the dashboard.
...
- **Strain rename import** — Upload **.csv**, **.xlsx**, or **.xlsm** via **Strains -> Import spreadsheet**. The app detects headers, lets admins remap columns interactively, previews how many purchase lots will be renamed for each supplier/current-strain pair, and then applies the rename to matching lots. This is a controlled rename workflow, not a standalone strain master-data import.
...
- **Role-Based Access** — Super Admin, User, Super Buyer, and Viewer roles, with `Settings -> Access Control` for role templates and per-user permission overrides.
...
Current extraction note: the booth-SOP rollout also relies on startup bootstrap to add the booth-session / booth-evidence schema. A backend restart is required before the new extraction workflow appears in production.
