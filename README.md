...
- **Downstream state tracking** — The shared run record now also carries the first structured Phase 2 downstream fields: pot-pour warm off-gas timing and stir count, THCA oven / milling / destination, and HTE off-gas / clean-dirty / Prescott / queue-routing decisions. This stabilizes the downstream data model before a dedicated post-processing workflow screen is introduced.
...
- **Standalone Extraction Lab App** — `standalone-extraction-lab-app/` now mirrors the extractor workflow with a touch-first reactor board, a `Scan / Enter Lot` surface, chargeable lot browser, `100 lbs` default charge preset, remembered reactor defaults, and a locked standalone run-execution screen with booth-SOP progression checkpoints from under-vacuum confirmation through shutdown and run completion. Extractors get a focused operator run screen with one primary action at a time; supervisors/managers get the broader review layout with full timing cards. After run completion, the same screen continues into a numbered guided downstream workflow for pathway selection, post-extraction handoff, wet-output confirmation, and branch-specific pot-pour or minor-run steps. The tablet exposes only the current checkpoint inputs and the next allowed action while the backend rejects future-step progression actions or premature future-step booth fields. `Settings -> Operational Parameters` can prepopulate that run screen's common defaults for blend, fill / flush counts, total fill / flush weights, baskets, and CRC blend.
...
- **Standalone extraction continuation** — The tablet run screen now carries the operator through the booth workflow checkpoints and then into the first post-extraction handoff actions without leaving the standalone app. Operators can select the downstream pathway and confirm the initial wet THCA / wet HTE outputs before later THCA / HTE-specific screens exist.
- **Standalone downstream capture** — The same tablet run screen now keeps going into the first structured downstream fields: pot-pour timing, THCA-path timing and destination, and HTE-path timing / decision / queue fields. The later downstream-specific workflow screen can now be built on top of real stored state instead of notes.
- **Guided downstream workflow** — The tablet `Open Run` screen now presents the downstream work as a numbered, pathway-driven sequence. Operators choose the branch, start post-extraction, enter wet THCA / wet HTE in Step 3, confirm the handoff, then work top-to-bottom through pot-pour or minor-run downstream steps without navigating away from the run. Start/confirm timestamps are recorded from the step actions, and post-extraction flow now uses step-local actions instead of a page-level save. Pending and completed steps collapse so the active step stays obvious.
