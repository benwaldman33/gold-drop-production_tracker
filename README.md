...
- **Standalone Extraction Lab App** — `standalone-extraction-lab-app/` now mirrors the extractor workflow with a touch-first reactor board, a `Scan / Enter Lot` surface, chargeable lot browser, `100 lbs` default charge preset, remembered reactor defaults, and a locked standalone run-execution screen with booth-SOP progression checkpoints from vacuum confirmation through shutdown and run completion. Extractors get a focused operator run screen with one primary action at a time; supervisors/managers get the broader review layout with full timing cards. After run completion, the same screen continues into a numbered guided downstream workflow for pathway selection, post-extraction handoff, wet-output confirmation, and branch-specific pot-pour or minor-run steps. The tablet exposes only the current checkpoint inputs and the next allowed action while the backend rejects future-step progression actions or premature future-step booth fields. `Settings -> Operational Parameters` can prepopulate that run screen's common defaults for blend, fill / flush counts, total fill / flush weights, baskets, and CRC blend.
- **Extraction booth SOP workflow** — booth execution is now modeled separately from the downstream business summary on `Run`. Operators advance in strict sequence through vacuum confirmation, solvent charge, soak, mixer, filter clear, pressurization, recovery, flush setup, temperature verification, flush solvent charge, flow-resumed confirmation, final purge, final clarity, shutdown checklist, and run completion. Future steps stay locked until the current step predicate is satisfied or a manager-approved bypass is applied.
- **Extraction booth evidence** — the standalone extraction app can now upload solvent-chiller and plate-temperature evidence photos against the current booth session, and the backend stores those uploads as booth evidence records tied to the run/session.
- **Extraction booth timing controls** — booth timers now carry configured target durations for primary soak, mixer, flush soak, and optional final purge. The standalone extraction app shows whether each timer is active, on target, or short against those configured targets.
