...
- **Deviation reasons + supervisor override controls** — off-target booth timing, exception paths, and booth-stage bypass requests now require an operator reason. Supervisors can explicitly `Approve Deviation` or `Require Rework` with a recorded reason on the dashboard notification card; only approved bypass requests unlock the one-step `Use Approved Bypass` action on the tablet.
