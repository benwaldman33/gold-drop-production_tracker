...
- **Standalone Extraction Lab App** — `standalone-extraction-lab-app/` now mirrors the extractor workflow with a touch-first reactor board, a `Scan / Enter Lot` surface, chargeable lot browser, `100 lbs` default charge preset, remembered reactor defaults, and a standalone run-execution screen with timer-based controls plus guided progression actions (`Start Run`, `Start Mixer`, `Stop Mixer`, `Start Flush`, `Stop Flush`, `Mark Run Complete`) before the optional main-app handoff. `Settings -> Operational Parameters` can now prepopulate that run screen's common defaults for blend, fill / flush counts, total fill / flush weights, baskets, and CRC blend.
...
- **Standalone extraction continuation** — The tablet run screen now carries the operator from `Start Run` through `Mark Run Complete`, and then into the first post-extraction handoff actions without leaving the standalone app. Operators can select the downstream pathway and confirm the initial wet THCA / wet HTE outputs before later THCA / HTE-specific screens exist.
