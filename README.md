...
- **Mixer timing safety alerts** — once primary soak starts, the mixer is expected within 3 minutes and should run about 5 minutes; the backend now issues a critical supervisor notification if mixer start is delayed or runtime exceeds 6 minutes, and escalates to an emergency-class Slack alert if the critical alert stays unacknowledged for another 3 minutes.
