...
- **Standalone Extraction Lab App** — `standalone-extraction-lab-app/` now mirrors the extractor workflow with a touch-first reactor board, a `Scan / Enter Lot` surface, chargeable lot browser, `100 lbs` default charge preset, remembered reactor defaults, and a locked standalone run-execution screen with booth-SOP progression checkpoints from under-vacuum confirmation through shutdown and run completion. Extractors get a focused **operator run dashboard** with one primary action at a time, a compact **run status strip**, and an always-visible **live timers panel** for all booth clocks; supervisors/managers get the broader review layout with full timing cards, phase rail, and prep/reset checklist. After run completion, the standalone app now routes operators into a dedicated `Downstream` queue/tab where they open the run in downstream mode for pathway selection, post-extraction handoff, wet-output confirmation, and branch-specific pot-pour or minor-run steps. The tablet exposes only the current checkpoint inputs and the next allowed action while the backend rejects future-step progression actions or premature future-step booth fields. `Settings -> Operational Parameters` can prepopulate that run screen's common defaults for blend, fill / flush counts, total fill / flush weights, baskets, and CRC blend.
- **Extraction booth SOP workflow** — booth execution is now modeled separately from the downstream business summary on `Run`. Operators advance in strict sequence through under-vacuum confirmation, solvent charge, soak, mixer start, mixer end, primary soak end confirmation, reactor bottom burp confirmation, filter clear, pressurization, recovery, flush setup, temperature verification, flush solvent charge, flow-resumed confirmation, final purge, final clarity, shutdown checklist, and run completion. Future steps stay locked until the current step predicate is satisfied or a manager-approved bypass is applied.
