...
- **Standalone Extraction Lab App** — `standalone-extraction-lab-app/` now mirrors the extractor workflow with a touch-first reactor board, a `Scan / Enter Lot` surface, chargeable lot browser, `100 lbs` default charge preset, remembered reactor defaults, and a standalone run-execution screen with timer-based controls before the optional main-app handoff. `Settings -> Operational Parameters` can now prepopulate that run screen's common defaults for blend, fill / flush counts, total fill / flush weights, baskets, and CRC blend.
