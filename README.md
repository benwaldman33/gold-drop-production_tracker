...
- **Standalone Extraction Lab App** — `standalone-extraction-lab-app/` now mirrors the extractor workflow with a touch-first reactor board, a `Scan / Enter Lot` surface, chargeable lot browser, `100 lbs` default charge preset, remembered reactor defaults, and a locked standalone run-execution screen with booth-SOP progression checkpoints from vacuum confirmation through shutdown and run completion. The tablet now exposes only the current checkpoint inputs and the next allowed action while the backend rejects future-step progression actions or premature future-step booth fields. `Settings -> Operational Parameters` can now prepopulate that run screen's common defaults for blend, fill / flush counts, total fill / flush weights, baskets, and CRC blend.
...
- **Guided downstream workflow** — The tablet `Open Run` screen now presents the downstream work as a numbered, pathway-driven sequence. Operators choose the branch, confirm the handoff, then work top-to-bottom through pot-pour or minor-run downstream steps without navigating away from the run.
