...
def test_mobile_extraction_reactor_cleared_frees_board():
    app = app_module.create_app()
    _set_mobile_workflows(app)
    supplier_id = _create_supplier(app, f"Cleared Supplier {gen_uuid()[:6]}")
    purchase_id = None
    lot_id = None
    charge_id = None
    try:
        with app.app_context():
            ...
            purchase = Purchase(
                ...
                testing_status="completed",
                clean_or_dirty="clean",
                batch_id=f"CLR-{gen_uuid()[:6]}",
            )
            db.session.add(purchase)
            db.session.flush()
            lot = PurchaseLot(
                purchase_id=purchase.id,
                strain_name="Cleared Strain",
                weight_lbs=100,
                remaining_weight_lbs=100,
                floor_state="reactor_staging",
                milled=True,
        ...
        with app.test_client() as client:
            ...
            before_cards = before.get_json()["data"]["reactor_cards"]
            reactor_one = next(card for card in before_cards if card["reactor_number"] == 1)
            assert reactor_one["state_key"] == "completed"
            assert any(action["target_state"] == "cleared" for action in reactor_one["current"]["available_actions"])
            ...
            after_cards = after.get_json()["data"]["reactor_cards"]
            reactor_one_after = next(card for card in after_cards if card["reactor_number"] == 1)
            assert reactor_one_after["state_key"] == "empty"
            assert reactor_one_after["current"] is None
    finally:
        with app.app_context():
            charge = db.session.get(ExtractionCharge, charge_id) if charge_id else None
            if charge:
