...
from services.purchases_journey import JOURNEY_STAGE_KEYS
...
def _login_viewer(client) -> None:
    # Ensure any remembered session from prior flows is cleared for this client.
    client.get("/logout", follow_redirects=False)
    resp = client.post(
        "/login",
        data={"username": "viewer", "password": "golddrop2026"},
        follow_redirects=False,
    )
    assert resp.status_code in (302, 303)
...
def test_archived_purchase_journey_requires_include_archived_flag():
    ...
    try:
        with app.test_client() as client:
            ...
            export_missing = client.get(f"/purchases/{purchase_id}/journey/export?format=json")
            assert export_missing.status_code in (302, 303)

            export_archived = client.get(f"/purchases/{purchase_id}/journey/export?include_archived=1&format=json")
            assert export_archived.status_code == 200
    finally:
        with app.app_context():
            p = db.session.get(Purchase, purchase_id)
            if p:
...
def test_purchase_journey_api_contract_shape_and_stage_order():
    app = app_module.app
    supplier = Supplier(name=f"Journey Contract {gen_uuid()[:8]}", is_active=True)
    purchase = Purchase(
        supplier_id="",
        purchase_date=date(2026, 4, 1),
        status="ordered",
        stated_weight_lbs=42,
    )
    with app.app_context():
        db.session.add(supplier)
        db.session.flush()
    ...
    try:
        with app.test_client() as client:
            ...
            assert res.status_code == 200
            payload = res.get_json()
            assert set(payload.keys()) == {"purchase_id", "batch_id", "status", "include_archived", "events"}

            stage_keys = [e["stage_key"] for e in payload["events"]]
            assert tuple(stage_keys) == JOURNEY_STAGE_KEYS

            for ev in payload["events"]:
                assert set(ev.keys()) == {"stage_key", "state", "started_at", "completed_at", "metrics", "links"}
                assert isinstance(ev["metrics"], dict)
                assert isinstance(ev["links"], list)
    finally:
        with app.app_context():
            p = db.session.get(Purchase, purchase_id)
            if p:
...
def test_non_admin_cannot_access_archived_journey_with_include_archived():
    app = app_module.app
    supplier = Supplier(name=f"Journey Auth {gen_uuid()[:8]}", is_active=True)
    purchase = Purchase(
        supplier_id="",
        purchase_date=date(2026, 4, 1),
        status="cancelled",
    ...
    try:
        with app.test_client() as client:
            ...
            api_res = client.get(f"/api/purchases/{purchase_id}/journey?include_archived=1")
            assert api_res.status_code == 404

            page_res = client.get(f"/purchases/{purchase_id}/journey?include_archived=1")
            assert page_res.status_code in (302, 303)
    finally:
        with app.app_context():
            p = db.session.get(Purchase, purchase_id)
            if p:
