...
- a shipped **Phase 3 guided downstream operator flow** in the standalone extraction app so operators can work through the downstream branch in a numbered, pathway-driven sequence while the main app remains the supervisor edit surface; Step 3 wet-output edits persist on `Confirm Initial Outputs` or explicit `Save Updates`
