...
- a shipped **standalone extraction operator UX refactor** so extractors see a focused one-action run screen during booth execution, supervisors/managers see the broader review layout, and the post-extraction handoff uses gated step cards with pathway selection, session start, wet-output confirmation, undo-before-confirm, and branch-specific downstream capture on the same `Open Run` screen
