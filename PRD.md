...
- a shipped **operator run status dashboard** in the standalone extraction app so floor operators see ambient run context while working the current checkpoint: a compact status strip (prep pills plus key readings) and an always-visible live timers panel for primary soak, mixer, flush soak, and final purge; the verbose phase rail and prep/reset checklist remain on the supervisor run screen
...
- a shipped **live booth timer clock layer** so active timer cards show continuously ticking elapsed/remaining (or over-target) time, and mixer control language reflects `Start Mixer` / `End Mixer` with a controlled `Restart Mixer` path before primary soak end confirmation; primary soak end and reactor bottom burp are explicit checkpoints before filter clear and pressurization
