...
- a shipped **live booth timer clock layer** so active timer cards show continuously ticking elapsed/remaining (or over-target) time, and mixer control language reflects `Start Mixer` / `End Mixer` with a controlled `Restart Mixer` path before filter clear
