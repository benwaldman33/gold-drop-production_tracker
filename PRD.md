...
**Operator workflow — purchase spreadsheet import:** Users with purchase edit access can upload **CSV or Excel** (`.csv`, `.xlsx`, `.xlsm`) via **Purchases → Import spreadsheet**; the system detects the header row, suggests a column mapping, lets the user adjust that mapping interactively, shows a **preview** with per-row validation, then commits selected rows into **Purchase** records (and optional **PurchaseLot** when strain / lot fields are mapped). The supported field set now covers the main purchase fields plus pipeline/testing and single-lot workflow fields. Staging uses a **server temp file** plus session token (not large cookie payloads). Duplicate **Batch ID** values are rejected; optional **auto-create suppliers** is supported.

**Operator workflow — supplier spreadsheet import:** Users with edit access can upload **CSV or Excel** (`.csv`, `.xlsx`, `.xlsm`) via **Suppliers → Import spreadsheet**; the system detects the header row, suggests a column mapping, lets the user adjust that mapping interactively, shows a duplicate-aware **preview**, and then commits selected rows into **Supplier** records. Exact case-insensitive name matches can be treated as updates when the operator enables that option; typo-close names remain warnings, not silent merges. Staging uses a **server temp file** plus session token.

**Operator workflow — strain spreadsheet import:** Users with edit access can upload **CSV or Excel** (`.csv`, `.xlsx`, `.xlsm`) via **Strains → Import spreadsheet**; the system detects the header row, suggests a column mapping, lets the user adjust that mapping interactively, shows a preview of how many matching lots will be renamed, and then commits selected rows as **bulk lot strain renames**. This is intentionally a rename workflow over `PurchaseLot.strain_name`, not a standalone strain master-data import. Staging uses a **server temp file** plus session token.

**Operator workflow — inventory spreadsheet import:** Users with purchase edit access can upload **CSV or Excel** (`.csv`, `.xlsx`, `.xlsm`) via **Inventory → Import spreadsheet**; the system detects the header row, suggests a column mapping, lets the user adjust that mapping interactively, shows a preview of matched lots by **tracking ID**, and then commits selected rows as **lot updates only**. This importer is intentionally constrained to the same safe lot-edit fields already exposed in the UI: strain name, potency, location, floor state, milled state, and notes. It does **not** create lots or rewrite tracking IDs, original lot weight, remaining pounds, or allocations. Staging uses a **server temp file** plus session token.
...
- a shipped **main-app load confirmation + action split** so lot charging now confirms lot/reactor/weight before submit and supports either `Record Load` (save load + stay in context) or `Record Load and Open Run`
...
- a shipped **Phase 1 post-extraction foundation** on the canonical run record, now launched through a dedicated standalone `Downstream` queue/tab, so operators can choose the downstream pathway (`100 lb pot pour` vs `200 lb minor run`), start the post-extraction handoff, and confirm the initial wet THCA / wet HTE outputs before the later THCA / HTE-specific workflow screens exist
- a shipped **Phase 2 downstream state layer** on the same canonical run record (pot-pour timing, THCA-path timing/destination, HTE-path timing/decision/queue state), exposed through the standalone `Downstream` workflow lane
- a shipped **Phase 3 guided downstream operator flow** in the standalone extraction app so operators can work through the downstream branch in a numbered, pathway-driven sequence while the main app remains the supervisor edit surface; Step 2/3 timestamps are action-stamped (`Start Post-Extraction`, `Confirm Initial Outputs`) and post-extraction no longer depends on page-level save actions
- a shipped **standalone extraction operator UX refactor** so extractors see a focused one-action run screen during booth execution, supervisors/managers see the broader review layout, and post-extraction handoff moves into a dedicated standalone `Downstream` lane with gated step cards (pathway selection, session start, wet-output confirmation, undo-before-confirm, and branch-specific downstream capture)
- a shipped **operator run status dashboard** in the standalone extraction app so floor operators see ambient run context while working the current checkpoint: a compact status strip (prep pills plus key readings) and an always-visible live timers panel for primary soak, mixer, flush soak, flush mixer, and final purge; the verbose phase rail and prep/reset checklist remain on the supervisor run screen
- a shipped **always-available booth evidence panel** so operators can capture camera/upload proof from any run checkpoint via a collapsible panel instead of waiting for phase-specific visibility
- a shipped **live booth timer clock layer** so active timer cards show continuously ticking elapsed/remaining (or over-target) time, and mixer control language reflects `Start Mixer` / `End Mixer` with a controlled `Restart Mixer` path before primary soak end confirmation; flush soak includes a flush-mixer cycle in the last minutes of soak; primary soak end and reactor bottom burp are explicit checkpoints before filter clear and pressurization
- a shipped **Step 3 wet-output submission hardening** so `Confirm Initial Outputs` consistently carries typed wet THCA/HTE values by keeping evidence upload controls out of nested form markup
...
- a shipped **Phase 5 destination-specific downstream workflow layer** so supervisors can now open dedicated queue pages for `GoldDrop production queue`, `Liquid Loud hold`, `Terp strip / CDT cage`, `HP base oil hold`, and `Distillate hold`, each with queue history and destination-specific actions instead of relying only on the generic routing board
...
- Structured execution detail fields for run / fill timing, biomass blend `% milled / % unmilled`, fill and flush counts / weights, stringer baskets, CRC blend, and execution notes
