...
def _load_purchase_for_journey_or_error(
    purchase_id: str,
    *,
    api: bool = False,
    action: str = "view",
    ...
    if purchase.deleted_at is not None and not include_archived:
        msg = (
            ...
            if api
            else f"This purchase is archived. Add include_archived=1 (super admin) to {action} its journey."
        )
...
def purchase_journey(purchase_id):
    purchase, include_archived, error = _load_purchase_for_journey_or_error(purchase_id, action="view")
    if error:
        return error
...
def purchase_journey_export(purchase_id):
    purchase, include_archived, error = _load_purchase_for_journey_or_error(purchase_id, action="export")
    if error:
        return error
