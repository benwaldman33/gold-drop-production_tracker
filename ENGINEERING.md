...
- **`gold_drop/purchases_module.py`** - purchases list/form/approval route logic delegated from `app.py`; inline list approvals in Purchases and Biomass Pipeline post through this module and preserve `return_to`
  ...
  - **`services/extraction_run.py`** - standalone extraction run defaults, booth-SOP progression actions, booth-session payload shaping, booth evidence counts/history, booth timing targets plus per-step timing-policy enforcement, booth-triggered supervisor notifications (including mixer start/runtime threshold alerts and emergency escalation when unacknowledged), the post-extraction handoff gates (`post_extraction_pathway`, downstream start, initial wet-output confirmation), and the downstream state fields for pot-pour, THCA-path, and HTE-path tracking that the tablet workflow renders
...
- **Label / scan surfaces**
  ...
  - `GET|POST /lots/<lot_id>/charge`:
    - main-app submit now confirms lot/reactor/weight client-side before POST
    - `submit_action=record_only` saves `ExtractionCharge` and returns to `return_to` context
    - `submit_action=open_run` saves `ExtractionCharge` and redirects to `runs/new`
...
- **Framework foundation:** `services/import_framework.py` now owns generic tabular-upload parsing (`.csv`, `.xlsx`, `.xlsm`), header normalization, header-row detection, and row extraction from a saved column mapping. The purchases flow is the first concrete user of that shared helper.
- **Purchase metadata + suggestions:** `purchase_import.py` now defines `PURCHASE_IMPORT_FIELDS` (friendly labels + sections + descriptions), header alias groups, mapping choices, and the staging helper `parse_purchase_spreadsheet_upload_for_mapping(...)`. The older `parse_purchase_spreadsheet_upload(...)` remains as a compatibility wrapper over the new staging/mapping flow.
- **Detection:** Scans the first **50** grid rows for a header line with **≥2** recognized columns; requires a **supplier** column. Max **2000** data rows after mapping.
- **Routes (Flask):** `GET/POST /purchases/import` (`purchase_import`, `@purchase_editor_required`), `GET/POST /purchases/import/preview`, `POST /purchases/import/commit`, `GET /purchases/import/sample.csv`. Upload writes a staging JSON file under `tempfile.gettempdir()` named `gdp_purchimp_<token>.json`; only a random token is stored in `session["purchase_import_token"]`.
- **Interactive preview:** the preview page now stores raw headers + current mapping in staging, lets the user remap each spreadsheet column to an allowed purchase field, and rebuilds validation results on every preview refresh. The same staged mapping is reused at commit time, so import does not depend on hidden form payloads.
- **Validation / commit:** `gold_drop/purchase_import_module.py` — `_purchase_import_validate_row`, `purchase_import_build_staged_rows`, `_purchase_import_commit_norm`; reuses `_maintain_purchase_inventory_lots`, purchase budget helpers from **`gold_drop/purchases.py`**, and `log_audit`. Imported rows do **not** set **`purchase_approved_at`**; if parsed **`status`** ∈ **`INVENTORY_ON_HAND_PURCHASE_STATUSES`**, commit forces **`ordered`** instead. Optional **Amount** → `total_cost`; **Paid date** / **Week** / **Payment method** folded into **notes**; **invoice** weight can fall back to **actual** weight; **purchase date** can fall back to **paid date** when purchase date is blank.
- **Field coverage:** the interactive mapper now supports the main purchase fields plus pipeline/testing fields (`availability_date`, `declared_weight_lbs`, `declared_price_per_lb`, `testing_timing`, `testing_status`, `testing_date`, `testing_notes`, `delivery_notes`) and single-lot fields (`strain`, `lot_weight_lbs`, `lot_potency_pct`, `lot_milled`, `lot_floor_state`, `lot_location`, `lot_notes`).
