...
## Runtime probes & diagnostics

- `GET /livez` — process liveness (returns `{"status":"ok"}` with `200`).
- `GET /readyz` — readiness (DB + app env), returns `200` on DB success and `503` on DB failure with payload:
  - `status`: `ok` / `degraded`
  - `db`: `ok` / `error`
  - `app_env`: normalized env value from `APP_ENV` / `FLASK_ENV`.
- `GET /healthz` — compatibility alias of readiness behavior.
- `GET /version` — deploy metadata for diagnostics:
  - `version` from `APP_VERSION` (fallback `dev`)
  - `app_env` from `APP_ENV` / `FLASK_ENV` (fallback `development`)
- `scripts/ops_preflight.py` now validates probe contracts and, in production-like envs, also checks required env vars (`SECRET_KEY`, `APP_VERSION`).

## Migration baseline tooling

- `manage.py` wires Flask-Migrate for DB lifecycle commands.
- Typical flow:
  - `python manage.py db init` (once)
  - `python manage.py db migrate -m "message"`
  - `python manage.py db upgrade`

## Demo-seed controls

- `_seed_demo_module_rows()` seeds one baseline row per core module/table.
- `SEED_DEMO_DATA` controls behavior:
  - unset → enabled in non-prod-like envs, disabled in `prod`/`production`/`staging`
  - explicit override via truthy/falsey env value.

## Local developer command shortcuts

- `make run` — run Flask app (`python app.py`)
- `make test` — run test suite (`pytest -q`)
- `make preflight` — run ops probe sanity check (`python scripts/ops_preflight.py`)
