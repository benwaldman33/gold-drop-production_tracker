...
def build_purchase_journey_payload(
    ...
    *,
    ...
    # Contract guard: ensure event ordering/identity stays stable for UI/API consumers.
    if tuple(ev.get("stage_key") for ev in events) != JOURNEY_STAGE_KEYS:
        raise RuntimeError("Journey event sequence does not match JOURNEY_STAGE_KEYS contract.")
