...
def run_new_view(root):
    ...
    if slack_prefill:
        if not root.current_user.can_slack_import:
            ...
            return root.redirect(root.url_for("dashboard"))
        display_run = root._hydrate_run_from_slack_prefill(slack_prefill, today)
...
def save_run(root, existing_run):
    ...
    try:
        ...
        paths = list(root._json_paths(run.hte_lab_result_paths_json))
        for rem in root.request.form.getlist("remove_hte_lab_paths[]"):
            if rem in paths:
                paths.remove(rem)
        new_files = root.request.files.getlist("hte_lab_files[]")
        if new_files:
            paths.extend(root._save_lab_files(new_files, prefix=f"hte-run-{run.id[:8]}"))
        ...
        for lid, lw in zip(lot_ids, lot_weights):
            if not lid or not lw:
                continue
            weight = float(lw)
            if weight <= 0:
                continue
            lot = root.db.session.get(root.PurchaseLot, lid)
            ...
            root.db.session.add(root.RunInput(run_id=run.id, lot_id=lid, weight_lbs=weight))
        ...
        if not existing_run and slack_meta and resolution:
            ...
            if need_supplier_materialize:
                sid = root._slack_resolution_materialize_supplier(resolution, slack_meta)
                if resolution.get("biomass_declared") and sid:
                    root._slack_resolution_create_declared_biomass(resolution, sid, slack_meta, run.run_date)

        audit_details = None
        if not existing_run and slack_meta and slack_meta.get("channel_id") and slack_meta.get("message_ts"):
            run.slack_channel_id = slack_meta["channel_id"]
            run.slack_message_ts = slack_meta["message_ts"]
            run.slack_import_applied_at = root.datetime.utcnow()
            audit_payload = {
                "slack_import": True,
    ...
    if run and run.deleted_at is None:
        for inp in run.inputs:
            ...
            if lot and lot.deleted_at is None:
                lot.remaining_weight_lbs += inp.weight_lbs
        run.deleted_at = root.datetime.utcnow()
