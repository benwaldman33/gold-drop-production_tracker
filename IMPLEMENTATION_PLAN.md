...
- Runtime hardening follow-through shipped:
  - `/livez`, `/readyz`, `/healthz`, `/version`
  - `scripts/ops_preflight.py` with production env-var checks
  - `manage.py` Flask-Migrate CLI scaffold
  - `SEED_DEMO_DATA` environment control for demo baseline seeding
