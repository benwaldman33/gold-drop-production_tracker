...
**Manager-facing note for the current release:** the app now includes the first derivative-lot genealogy foundation behind the scenes. Current day-to-day workflows still use Purchases, Inventory, Runs, and Downstream Queues the same way, but the system can now bridge biomass lots into first-class material genealogy records in preparation for future ancestry / descendant reporting.
